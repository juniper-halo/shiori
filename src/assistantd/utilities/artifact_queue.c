...
int assistantd_llm_response_queue_enqueue(
  ...
  if (queue->size >= ASSISTANTD_SUPERVISOR_LLM_RESPONSE_QUEUE_CAPACITY) {
    return 0;
  }

  queue->items[queue->tail] = *response;
  queue->tail = (queue->tail + 1) % ASSISTANTD_SUPERVISOR_LLM_RESPONSE_QUEUE_CAPACITY;
  queue->size++;
  return 1;
}
...
int assistantd_llm_response_queue_dequeue(
  ...
  if (queue == NULL || response == NULL || queue->size == 0) {
    return 0;
  }

  *response = queue->items[queue->head];
  memset(&queue->items[queue->head], 0, sizeof(queue->items[queue->head]));
  queue->head = (queue->head + 1) % ASSISTANTD_SUPERVISOR_LLM_RESPONSE_QUEUE_CAPACITY;
  queue->size--;
  return 1;
}
